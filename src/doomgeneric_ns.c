...
{
	/*int wakeUpIn = GetTickCount() + ms;
	while (wakeUpIn <= GetTickCount())
	...
	switch (messageType)
	{
		...
		case EVENT_PAINT:
		{
			if (drawFrameNow)
			{
				...
				{
					//draw frame:
					VidBlitImage(&image, 3, 2 + TITLE_BAR_HEIGHT);
				}
			}
			drawFrameNow = false;
			break;
...
void WindowDeconstruct()//called AtExit
{
	if (g_pWindow)
	{
		...
		//The EVENT_DESTROY handler sets g_pWindow to NULL, so stop handling messages when g_pWindow is NULL.
		while (g_pWindow)
			HandleMessages(g_pWindow);
	}
}
...
bool WindowInit()
{
	Window* pWindow = CreateWindow ("doom", 150, 150, DOOMGENERIC_RESX + 6, DOOMGENERIC_RESY + 4 + TITLE_BAR_HEIGHT, WndProc, 0);
