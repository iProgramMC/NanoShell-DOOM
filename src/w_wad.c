...
static void ExtendLumpInfo(unsigned int newnumlumps)
{
    ...
    //newlumpinfo = calloc(newnumlumps, sizeof(lumpinfo_t));//UB?
    newlumpinfo = malloc(newnumlumps * sizeof(lumpinfo_t));

    if (newlumpinfo == NULL)
    {
		I_Error ("Couldn't realloc lumpinfo! LumpInfo count: %d, sizeof: %d (total bytes:%d)", newnumlumps, sizeof(lumpinfo_t), newnumlumps*sizeof(lumpinfo_t));
    }
...
wad_file_t* g_filesToClose[400];
int         g_filesToCloseCount = 0;

//it's important no accessing this pointer happens, because it may have been freed, we just want to get rid of its entry
void ns$OnCloseFile(wad_file_t* pFile)
{
	for (int i = 0; i < g_filesToCloseCount; i++)
	{
		if (g_filesToClose[i] == pFile)
		{
			g_filesToClose[i] =  NULL;
		}
	}
}

void ns$CloseAllFiles (void)
{
	for (int i = 0; i < g_filesToCloseCount; i++)
	{
		if (g_filesToClose[i] != NULL)
		{
			W_CloseFile(g_filesToClose[i]);
		}
	}
}
...
wad_file_t *W_AddFile (char *filename)
{
	...
	for (int i = 0; i < g_filesToCloseCount; i++)
	{
		if (g_filesToClose[i] == NULL)
		{
			g_filesToClose[i]  = wad_file;
			set = true;
		}
	}
	if (set == false)
	{
		g_filesToClose[g_filesToCloseCount++] = wad_file;
	}
...
void I_EndRead (void);//i_video.c -- forward decl
//
// W_ReadLump
